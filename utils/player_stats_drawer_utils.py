...
def draw_player_stats(output_video_frames, player_stats):
    for index, row in player_stats.iterrows():
        ...
        player_1_speed = row['player_1_speed']
        player_2_speed = row['player_2_speed']

        player_1_distace = row['player_1_distance']
        player_2_distace = row['player_2_distance']
        ...
        output_video_frames[index] = cv2.putText(output_video_frames[index], text, (start_x + 10, start_y + 160),
                                                 cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 255, 255), 1)
        text = f"{player_1_distace:.1f} m    {player_2_distace:.1f} m"
